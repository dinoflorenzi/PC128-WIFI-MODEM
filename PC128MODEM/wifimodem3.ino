...
#define TX_BUF_SIZE 1024    // Buffer where to read from serial before writing to TCP
...
void waitCTS()
{
  ...
  while(true)
  {
    if( digitalRead(CTS_PIN) == HIGH) break;
    yield();
  }
...
void defaultEEPROM() {
  ...
  EEPROM.write(FLOW_CONTROL_ADDRESS, 0x01);
...
void welcome() {
  waitCTS();
  Serial.println();
  Serial.println("WIFI PC128 BUILD " + build + " BY @DINOFLORENZI");
...
// Definizioni per i buffer dedicati alla trasmissione TCP->Serial
#define TCP_RX_BUF_SIZE 256
#define TCP_TX_BUF_SIZE 512



// Funzione ottimizzata per trasmettere dati da TCP a Serial

void transmitTCPToSerial() {
uint8_t rxBuffer[TCP_RX_BUF_SIZE];
    uint16_t rxLen = 0;
    unsigned long startTime = millis();

    while (rxLen < TCP_RX_BUF_SIZE && millis() - startTime < 10) { // Attendi max 10ms per accumulare dati
        int availableBytes = tcpClient.available();
        if (availableBytes > 0) {
            int bytesToRead = min(availableBytes, TCP_RX_BUF_SIZE - rxLen);
            tcpClient.readBytes(&rxBuffer[rxLen], bytesToRead);
            rxLen += bytesToRead;
        }
    }

    if (rxLen > 0) {
        waitCTS();  // Attendi che il dispositivo seriale sia pronto (se necessario)
...
void transmitTCPToSerial() {
  uint8_t rxBuffer[TCP_RX_BUF_SIZE];
  uint8_t txBuffer[TCP_TX_BUF_SIZE];
  size_t rxLen = 0, txLen = 0;

  // Legge i dati disponibili dal tcpClient fino a TCP_RX_BUF_SIZE
  while (tcpClient.available() && rxLen < TCP_RX_BUF_SIZE) {
    rxBuffer[rxLen++] = tcpClient.read();
  }

  // Se sono stati letti dati, li processa
  if (rxLen > 0) {
    for (size_t i = 0; i < rxLen; i++) {
      uint8_t byte = rxBuffer[i];
      // Se siamo in modalità Telnet e troviamo 0xff, lo raddoppiamo
      if (telnet && byte == 0xff) {
        if (txLen < TCP_TX_BUF_SIZE - 1) {  // Verifica che ci sia spazio per due byte
          txBuffer[txLen++] = 0xff;
          txBuffer[txLen++] = 0xff;
        }
      }
      else {
        if (txLen < TCP_TX_BUF_SIZE) {
          txBuffer[txLen++] = byte;
        }
      }
    }
    // Scrive l'intero buffer su Serial in una singola chiamata
waitCTS();
...
void loop() {
  ...
  // Nuova connessione in ingresso sul socket del server TCP
  ...
  /**** AT command mode ****/
  if (cmdMode == true) {
    // In modalità comando, non si scambia con TCP ma si raccolgono i caratteri in una stringa
    if (Serial.available()) {
      char chr = Serial.read();

      if (petTranslate == true)
        // Corregge la codifica da PETSCII a ASCII
        if (chr > 127) chr -= 128;
      else
        // Converte da PETSCII maiuscolo a ASCII minuscolo (C64) in modalità comando
        if ((chr >= 193) && (chr <= 218)) chr -= 96;

      // Se il carattere è un ritorno, newline o carriage return, esegue il comando
      if ((chr == '\n') || (chr == '\r')) {
        command();
      }
      // Backspace o delete elimina l'ultimo carattere
  ...
  /**** Connected mode ****/
    ...
    // Trasmissione da Serial a TCP
    // (Eventuale gestione del controllo di flusso qui commentata)
    if (Serial.available()) {
      // In modalità Telnet, per sicurezza lasciamo metà del buffer libero per eventuali escape
      int max_buf_size;
      if (telnet == true)
      ...
      // Legge dalla seriale, fino a max_buf_size byte
      size_t len = std::min(Serial.available(), max_buf_size);
      Serial.readBytes(&txBuf[0], len);

      // Rileva la sequenza "+++" per entrare in modalità comando
      ...
      // Se in modalità Telnet, raddoppia ogni 0xff nel buffer (approccio in-place)
      if (telnet == true) {
      ...
      // Correzione della codifica PET da PETSCII ad ASCII se richiesto
      if (petTranslate == true) {
        for (int i = len - 1; i >= 0; i--) {
          if (txBuf[i] > 127) txBuf[i] -= 128;
        }
      }
      // Invia il buffer elaborato su TCP
      tcpClient.write(&txBuf[0], len);
      yield();
    }

    // Trasmissione da TCP a Serial con la funzione ottimizzata
    transmitTCPToSerial();
  }

  // Se sono stati ricevuti "+++" come ultimi byte dalla seriale e trascorre più di 1 secondo
  if (plusCount >= 3) {
  ...
  // Se la connessione TCP è terminata e non siamo in modalità comando, passa in modalità comando
  ...
  // Gestione del LED TX/RX: spegne il LED se è acceso da abbastanza tempo
  if (millis() - ledTime > LED_TIME)
    digitalWrite(LED_PIN, !digitalRead(LED_PIN)); // alterna lo stato del LED
}